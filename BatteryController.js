...
A negative run level means we will be storing to batteries using chargers.
A positive run level means we will be generating AC power  using the uInverters.

The battery quantum of power is the amount of power consumed by each charger.
The uInveter quantum of power is the amoun tof power generated by each uInerter.
*/
class BatteryController {
  constructor(ip){
    this.ip=ip;
    this.runLevel=0; // start idle
    ...
    this.WMismatchThresh=50; // how many W can differ between a known consW-prodW and a meter reported total consumption
    this.Nui=6; // Limit the total number of uInverters available
    this.Nbc=-this.Nui; // Limit the total number of battery chargers available (-count)
  ...
  */
  ...
  logState(){
    console.log(''+this.consW+'-'+this.prodW+'='+(this.consW-this.prodW)+'~='+this.netW+' W : runLevel '+this.runLevel);
  }
